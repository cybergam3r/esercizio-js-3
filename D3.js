...
let crewMass = 0;
let numerato = 0;
while (numerato < starWarsCharacters.length) {
  crewMass += starWarsCharacters[numerato].mass;
  numerato++;
}
...
let mass1000 = 1000;
console.log(crewMass);
if (crewMass >= mass1000) {
  console.log("DANGER! OVERLOAD ALERT: escape from ship now!");
} else {
  if (mass900 < crewMass && crewMass < mass1000) {
    console.log("Critical Load: Over 900");
  } else {
    if (mass700 < crewMass && crewMass < mass900) {
      console.log("Warning: Load is over 700");
    } else {
      if (mass500 < crewMass && crewMass < mass700) {
        console.log("Ship is half loaded");
      } else {
        if (mass500 < crewMass || mass500 >= crewMass) {
